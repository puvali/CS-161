...
(defun neg-literals (start i end)
  (cond ((> i end) nil)
	(t (cons (list (* start -1) (* i -1)) (neg-literals start (+ i 1) end)))))

(defun literals (start i end)
  (cond ((> i end) nil)
	(t (cons (list start i) (literals start (+ i 1) end)))))

(defun amo-helper (start end)
  (cond ((= start end) nil)
	(t (append (neg-literals start (+ start 1) end) (amo-helper (+ start 1) end)))))

(defun ec-helper (e i k)
  (cond ((null e) nil)
	((< i 1) nil)
	((> i k) nil)
	(t (append (list (list (* (node2var (first e) i k) -1) (* (node2var (second e) i k) -1)))
		   (ec-helper e (+ i 1) k)))))
...
(defun at-most-one-color (n c k)
  (cond
   ((= c k) nil)
   (t (let ((hlist (at-least-one-color n c k)))
	(amo-helper (first hlist) (car (last hlist))))))
   )
...
(defun generate-node-clauses (n k)
  (let ((alo (at-least-one-color n 1 k)) (amo (at-most-one-color n 1 k)))
    (append (list alo) amo))
  )
